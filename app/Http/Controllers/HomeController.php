...
class HomeController extends Controller
{
    ...
    public function index(): Factory|View|Application {
        //  Get Latest Products
        $new = Product::with('subCategory')->join('admins', 'admins.user_id', 'seller_id')
            ->select('products.*', 'admins.username', 'section.title AS section')
            ->join('categories AS cat', 'products.category_id', '=', 'cat.id')
            ->join('categories AS section', 'cat.section_id', '=', 'section.id')->where(['products.status' => 1])
            ->orderByDesc('products.id')->limit(12)->get();

        $data = [
            'featuredProducts' => Product::with('subCategory', 'brand', 'seller')->where('products.status', 1)
                ->where('is_featured', 'Yes')->get(),
            'newProducts'      => $new,
            'topProducts'      => Product::with('subCategory')->where('status', 1)->orderByDesc('id')->limit(10)->get()
                ->shuffle(),
