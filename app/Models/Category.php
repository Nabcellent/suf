...
class Category extends Model
{
    public static function sections(): mixed {
        $getCategories = self::where(['category_id' => null, 'sub_category_id' => null])
    ...
    public function categories(): HasMany
    {
        return $this->hasMany(__CLASS__, 'category_id')
            ->where(['sub_category_id' => null, 'status' => 1])
            ->with('subCategories');
    }

    public function subCategories(): HasMany
    {
        return $this->hasMany(__CLASS__, 'sub_category_id')->where('status', 1);
    }
